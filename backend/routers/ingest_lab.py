...
router = APIRouter(prefix="/ingest/lab", tags=["Ingest: Lab"])
...
@router.post(
    ...
    summary="Ingest Lab Health Data",
    description="Post health data from a laboratory facility to the UHDE."
)
async def ingest_lab_data(
    ...
    """
    Ingest laboratory health data into the Smart Health system.
    ...
    try:
        ...
        return HealthIngestResponse(
            ...
            message=f"Lab record from {payload.source_name} successfully processed",
        ...
        raise HTTPException(status_code=500, detail=f"Failed to process lab record: {str(e)}")


@router.get("/health", summary="Health Check")
async def health_check() -> Dict[str, str]:
    """Health check endpoint for the lab ingest router."""
    return {
        "status": "ok",
        "service": "Lab Ingest API",
        "timestamp": datetime.utcnow().isoformat()
    }
