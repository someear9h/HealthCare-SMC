...
router = APIRouter(prefix="/ingest/ambulance", tags=["Ingest: Ambulance"])
...
@router.post(
    ...
    summary="Ingest Ambulance Health Data",
    description="Post health data from ambulance services to the UHDE."
)
async def ingest_ambulance_data(
    payload: HealthIngestPayload,
    background_tasks: BackgroundTasks
) -> HealthIngestResponse:
    """
    Ingest ambulance health data into the Smart Health system.
    ...
    try:
        ...
        return HealthIngestResponse(
            ...
            message=f"Ambulance record from {payload.source_name} successfully processed",
        ...
        raise HTTPException(status_code=500, detail=f"Failed to process ambulance record: {str(e)}")


@router.get("/health", summary="Health Check")
async def health_check() -> Dict[str, str]:
    """Health check endpoint for the ambulance ingest router."""
    return {
        "status": "ok",
        "service": "Ambulance Ingest API",
        "timestamp": datetime.utcnow().isoformat()
    }
